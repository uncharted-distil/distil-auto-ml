...
def setup_logging(logging_level, log_file="distil.log", system_version="UNSET"):
    ...
    sys_string = "%(levelname)s - %(asctime)s - v-{} - %(name)s - %(message)s".format(
        system_version
    )
...
def make_preds_filename(task_id, output_key=None, output_type=None):
    ...
    if output_type == ValueType.PARQUET_URI:
        ...
        extension = "csv"
        logger.warn(
            f"unhandled output type '{output_type}'.  Default to '{ValueType.CSV_URI}'"
        )

    if output_key is not None:
        path = pathlib.Path(
            config.OUTPUT_DIR, f"{format(task_id)}_{output_key}.{extension}"
        ).resolve()
    else:
        path = pathlib.Path(
            config.OUTPUT_DIR, f"{format(task_id)}.{extension}"
        ).resolve()
