...
def create_pipeline(metric: str,
    ...
    step.add_hyperparameter('sample_size', ArgumentType.VALUE, 0.75)
    ...
    step.add_argument(name='inputs', argument_type=ArgumentType.CONTAINER, data_reference='steps.1.produce_collection')
    step.add_output('produce')
    audio_pipeline.add_step(step)
    ...
    step.add_argument(name='reference', argument_type=ArgumentType.CONTAINER, data_reference='steps.2.produce')
    step.add_output('produce')
    audio_pipeline.add_step(step)
