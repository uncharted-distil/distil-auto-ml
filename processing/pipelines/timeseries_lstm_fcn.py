...
def create_pipeline(
    metric: str, min_meta: bool = False, resolver: Optional[Resolver] = None
) -> Pipeline:
    ...
    step_0 = PrimitiveStep(
        primitive_description=DistilTimeSeriesFormatter.metadata.query(),
        resolver=resolver,
    )
    step_0.add_argument(
    ...
    step_1 = PrimitiveStep(
        DatasetToDataFramePrimitive.metadata.query(), resolver=resolver
    )
    step_1.add_argument(
        name="inputs",
    ...
    step_4 = PrimitiveStep(
        ExtractColumnsBySemanticTypesPrimitive.metadata.query(), resolver=resolver
    )
    step_4.add_argument(
        name="inputs",
        argument_type=ArgumentType.CONTAINER,
    ...
    # Step 6: LSTM_FCN
    step_5 = PrimitiveStep(LSTM_FCN.metadata.query(), resolver=resolver)
    step_5.add_argument(
        name="inputs",
        argument_type=ArgumentType.CONTAINER,
        data_reference=input_val.format(ts_parse_step),
