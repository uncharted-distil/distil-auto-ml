...
from distil.primitives.collaborative_filtering_link_prediction import (
    CollaborativeFilteringPrimitive,
)


def create_pipeline(
    metric: str, resolver: Optional[Resolver] = None, min_meta: bool = False
) -> Pipeline:
    previous_step = 0
    ...
    step = PrimitiveStep(
        primitive_description=CollaborativeFilteringPrimitive.metadata.query(),
        resolver=resolver,
    )
    step.add_argument(
        name="inputs",
