...
def create_pipeline(
    ...
    if include_aug:
        ...
        step = PrimitiveStep(
            primitive_description=DataMartAugmentPrimitive.metadata.query(),
            resolver=resolver,
        )
        step.add_argument(
            name="inputs",
        ...
        step.add_hyperparameter(
            "search_result", ArgumentType.VALUE, join_result.serialize()
        )
        tabular_pipeline.add_step(step)
    else:
        logger.warn(
            "Datamart did not return result for input dataset - proceeding with baseline dataset"
        )
    ...
    step.add_argument(
        name="inputs", argument_type=ArgumentType.CONTAINER, data_reference=data_ref
    )
    step.add_output("produce")
    tabular_pipeline.add_step(step)
    ...
    step = PrimitiveStep(
        primitive_description=OneHotEncoderPrimitive.metadata.query(), resolver=resolver
    )
    step.add_argument(
        name="inputs",
        argument_type=ArgumentType.CONTAINER,
    ...
    step = PrimitiveStep(
        primitive_description=SKImputer.SKImputer.metadata.query(), resolver=resolver
    )
    step.add_argument(
        name="inputs",
        argument_type=ArgumentType.CONTAINER,
    ...
    if df_cols > max_cols:
        step = PrimitiveStep(
            primitive_description=SKPCA.SKPCA.metadata.query(), resolver=resolver
        )
        step.add_argument(
            name="inputs",
            argument_type=ArgumentType.CONTAINER,
...
def _query_datamart(
    keywords: List[Any], dataset: Optional[container.Dataset]
) -> Tuple[datamart.DatamartSearchResult, int, int]:
    ...
    query = datamart.DatamartQuery(keywords=list(keywords_list), variables=[])
    ...
    for result in results:
        ...
        try:
            ...
            logger.warn(
                f"Datamart failed to augment using dataset {join_with} column {join_col} - trying next result"
            )
            logger.exception(e)
            error = True
        if not error:
    ...
    for result in results:
        ...
        if result.get_augment_hint():
            ...
            print(
                "Right Columns: %s"
                % str(
                    result.get_json_metadata()["augmentation"]["right_columns_names"]
                ),
                file=sys.__stdout__,
            )
        else:
