...
python -m dummy_ta3.dummy_ta3 -p ./seed_datasets_current/$DATASET/TRAIN/problem_TRAIN/problemDoc.json -d ./seed_datasets_current -e 0.0.0.0 -t 45043 --time-bound-search 600
...
#  eval $($line | sed 's/^/export /'
    ...
    if test -f "$FILE"; then
      python -m d3m runtime \
          ...
          fit-score \
          -p ${D3MOUTPUTDIR}/${SEARCH_ID}/pipelines_ranked/$PIPELINE_ID.json \
